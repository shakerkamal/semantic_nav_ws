...
class NavigationOrchestrator(Node):
    def __init__(self):
        ...
        # DEFAULT IS 'spin' ON PURPOSE. This orchestrator is SHARED with the TB3
    ...
    def _rescan_confirm_and_validate(self, target, barrier_xy, clear_radius) -> bool:
        ...
        Each poll: re-observe so SLAM + costmap re-sense the passage (by default
        WITHOUT moving -- the standoff already faces the barrier, and rtabmap's
        map_always_update refreshes the grid while parked; see _reobserve), clear
        stale marks, then GATE on grounded perception -- a valid plan AND the
        responsible barrier's footprint reading CLEAR on the costmap.
    ...
    def _reobserve(self, poll: int = 0) -> None:
        ...
        mode = self._up_front_reobserve_mode

        if mode == 'spin':          # TB3 default: the ORIGINAL sweep, every poll
            self._sweep_spin()
            return
        ...
        if poll == 0:
            self._log_stage_info(
                "UP_FRONT",
                f"Re-observing from the standoff WITHOUT moving "
    ...
    def _sweep_spin(self) -> None:
        ...
        This is what the TB3 stack runs and what the locked Ch7 results were
        collected with, so it is left exactly as it was — do NOT "improve" it here.
        It is imperfect (Nav2's Spin overshoots in whichever direction it turns, and
